...
void DoTurn(const PlanetWars& pw) {
  int planet_count = pw.Planets().size();
  int distances[planet_count][planet_count];
  for(int i = 0; i < planet_count; i++){
     distances[i][i] = 999999;
     for(int j = i+1; j < planet_count; j++){
       distances[i][j] = distances[j][i] = pw.Distance(i,j);
     }
  }
 
  // (1) If we currently have a fleet in flight, just do nothing.
  if (pw.MyFleets().size() >= pw.MyPlanets().size()) {
    return;
  }
  ...
  for (int i = 0; i < not_my_planets.size(); ++i) {
    ...
    double score = distances[source][p.PlanetID()];
    if (score < dest_score) {
      dest_score = score;
      dest = p.PlanetID();
    }
