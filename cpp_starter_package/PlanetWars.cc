...
#ifdef SYSLOG
  syslog (LOG_NOTICE, "Move %d (of %d) Fleets from planet %d to planet %d", num_ships, GetPlanet(source_planet).NumShips(), source_planet, destination_planet);
  if(GetPlanet(source_planet).NumShips() < num_ships) 
  syslog (LOG_NOTICE, "Iligal Move");
#endif 
  Planet & p = (Planet &) planets_[source_planet];
  p.RemoveShips(num_ships);
  
  int distance = (int) Distance(source_planet, destination_planet);
  Fleet f(planets_[source_planet].Owner(), 
	  num_ships, 
	  source_planet, 
	  destination_planet, 
	  distance, 
	  distance);
  ((std::vector<Fleet>)fleets_).push_back(f);
  //Planet p2do(p.PlanetID(), p.Owner(), p.NumShips() - num_ships, p.GrowthRate(), p.X(), p.Y());
  //planets_.erase(planets_.begin() + source_planet);
  //planets_.insert(planets_.begin() + source_planet,p2do)
  //for(int j = 0; j < planets_.size(); j++){
  //  const Planet p = planets_[j];
  //  if(j == source_planet){
  //    Planet p2do(p.PlanetID(), p.Owner(), p.NumShips() - num_ships, p.GrowthRate(), p.X(), p.Y());
  //    new_planets_.push_back(p2do);
  //  }else{
  //    new_planets_.push_back(p);
  //  }
  //}
